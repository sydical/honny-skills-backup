...
# EXIF 注入配置（可选）
EXIF_ENABLED = os.environ.get('EXIF_ENABLED', 'true').lower() == 'true'
EXIF_PHONE_MODEL = os.environ.get('EXIF_PHONE_MODEL', 'iPhone 16')
EXIF_PHONE_MAKE = os.environ.get('EXIF_PHONE_MAKE', 'Apple')
EXIF_GPS_CITY = os.environ.get('EXIF_GPS_CITY', '福州')
...
class HonnyPhotoV2:
    ...
    def inject_exif(self, image_path):
        """"为图片注入手机相机 EXIF 信息"""
        if not EXIF_ENABLED:
            return image_path
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "inject_exif", 
                os.path.join(SKILL_DIR, '..', 'phone-exif', 'scripts', 'inject_exif.py')
            )
            exif_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(exif_module)
            return exif_module.inject_exif(image_path, image_path, EXIF_PHONE_MODEL, EXIF_PHONE_MAKE, EXIF_GPS_CITY)
    ...
    def run(self, prompt, user_dir=None, output_dir=None, optimize=True, iteration=1, inject_exif_flag=True):
        """运行完整流程"""
        ...
        if output_dir and image_url:
            ...
            output_path = os.path.join(output_dir, f"honny_{timestamp}.jpg")
            self.download_and_save(image_url, output_path)
            result["local_path"] = output_path
            
            # 5. 注入 EXIF（如启用）
            if inject_exif_flag and EXIF_ENABLED:
                print(f"📱 注入 EXIF 信息...")
                final_path = self.inject_exif(output_path)
                result["local_path"] = final_path
                lat, lon = GPS_COORDS.get(EXIF_GPS_CITY, GPS_COORDS["福州"])
                print(f"✅ EXIF 注入完成!")
                print(f"📱 机型: {EXIF_PHONE_MAKE} {EXIF_PHONE_MODEL}")
                print(f"📅 拍摄时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"📍 GPS: {EXIF_GPS_CITY} ({lat}°N, {lon}°E)")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        result = generator.run(prompt, output_dir=output_dir)
        print(f"\n🎉 生成成功!")
