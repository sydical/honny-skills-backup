...
description: >
  Honny 多图生成技能 - 一致性模特同场景不同表情不同动作套图生成。
  支持上传参考图（模特图），生成同模特在同场景下的不同表情、不同动作的四宫格图片。
  任务间隔5分钟，避免API并发限制。

  触发关键词：多图、套图、同场景不同表情、模特多动作、一致性套图
allowed-tools:
  - read
  - write
  - edit
...
使用 RunningHub 工作流进行一致性模特同场景多图生成，支持同一场景下不同表情、不同动作的多张套图。
...
| **多图生成** | 一次生成多张图片（根据 RunningHub 工作流返回的结果而定） |
| **任务间隔** | 每任务间隔5分钟，避免API并发限制 |
| **EXIF 注入** | 自动注入 参考图 同样的EXIF |
